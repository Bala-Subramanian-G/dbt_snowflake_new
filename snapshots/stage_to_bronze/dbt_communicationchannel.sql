-- snapshots/stage_to_bronze/dbt_communicationchannel.sql
{% snapshot dbt_communicationchannel %}
    {{
        config(
...
(
    ...
    from {{ ref('stg_communicationchannel') }} 
)
select
        {{ stage_columns('STG_COMMUNICATIONCHANNEL') }},
